...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            ...
            // If the invoice has a subscription, also cancel it if it's incomplete
            if (invoice.subscription) {
                try {
                    ...
                    if (subscription.status === 'incomplete' || subscription.status === 'past_due') {
                        ...
                        logger.info('Cancelled related subscription', {
                            subscriptionId: subscription.id,
                            userId: user.id
                        });
                    }
                } catch (subError) {
                    logger.error('Failed to cancel related subscription', {
